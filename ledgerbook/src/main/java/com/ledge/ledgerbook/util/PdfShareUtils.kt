...
object PdfShareUtils {
    ...
    fun shareEntry(context: Context, item: LedgerViewModel.LedgerItemVM) {
        val doc = PdfDocument()
        var pageNumber = 1
        ...
        // Header
        lineL("Ledger Receipt", title, 24f)
        lineL("Date: ${dateFmt.format(Date())}", text)
        y += 6f
        lineL("Customer: ${item.name}", label)
        y += 6f
    ...
    fun shareGroup(context: Context, name: String, items: List<LedgerViewModel.LedgerItemVM>) {
        val doc = PdfDocument()
        var pageNumber = 1
        var page = doc.startPage(PdfDocument.PageInfo.Builder(PAGE_WIDTH, PAGE_HEIGHT, pageNumber).create())
        var canvas: Canvas = page.canvas
        ...
        lineL("Ledger Statement", title, 24f)
        lineL("Customer: $name", header)
        lineL("Date: ${dateFmt.format(Date())}")
        y += 6f
        lineL("Total Lend: ${CurrencyFormatter.formatInr(lendTotal)}")
        lineL("Total Borrow: ${CurrencyFormatter.formatInr(borrowTotal)}")
        lineL("Net: ${CurrencyFormatter.formatInr(net)}", header)
        ...
        fun drawTable(titleText: String, rows: List<LedgerViewModel.LedgerItemVM>) {
            ...
            val subtotalValues = listOf(
                "Subtotal", // under Date column as label
        ...
        y += 24f
        drawTable("BORROW", borrow)

        // Footer note
        ensureSpace(24f)
        y += 6f
        lineL("Generated by Ledger Book", text, 16f)
