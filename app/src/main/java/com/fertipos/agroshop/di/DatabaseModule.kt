...
object DatabaseModule {
    ...
    private val MIGRATION_11_12 = object : Migration(11, 12) {
        override fun migrate(database: SupportSQLiteDatabase) {
            // Add isSupplier flag to customers, default to 0 (false)
            database.execSQL("ALTER TABLE customers ADD COLUMN isSupplier INTEGER NOT NULL DEFAULT 0")
        }
    }

    private val MIGRATION_12_13 = object : Migration(12, 13) {
        override fun migrate(database: SupportSQLiteDatabase) {
            // Add unitsCsv to company_profile with default preserving existing behavior
            database.execSQL(
                "ALTER TABLE company_profile ADD COLUMN unitsCsv TEXT NOT NULL DEFAULT 'Kg,Pcs,L'"
            )
        }
    }
